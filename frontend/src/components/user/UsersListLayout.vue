<template>
    <section>
        ...
        <div class="select" v-if="showSelected">
            <div class="select--wrapper">
                <div class="select--container">
                    <label for="role-select">Le rôle de {{ userToChange.name }} est {{ userToChange.roles[0] }}. Voulez
                        vous le modifier ?</label>
        ...
        <div class="select" v-if="showDeleted">
            <div class="select--wrapper">
                <div class="select--container">
                    <label for="role-select">Etes vous sûr de vouloir supprimer le compte de {{ userToDelete.name }}
                        ?</label>
...
export default {
    ...
    methods: {
        ...
        async deleteById(user) {
            ...
            if (response) {
                this.succesUpdate.push("Le compte de " + response.previous.name + " a bien été supprimé");
                setTimeout(() => {
                    this.succesUpdate = [];
                }, 5000);
            } else {
        ...
        async updateRole() {
            ...
            if (this.$store.getters.getRole === "administrator") {
                ...
                if (response.id) {
                    // console.log(response);
                    this.users = await UserLoginService.findAll();
                    this.succesUpdate.push("Le rôle de " + response.name + " a bien été modifié");
                    setTimeout(() => {
                        this.succesUpdate = [];
                    }, 5000);
                } else {
